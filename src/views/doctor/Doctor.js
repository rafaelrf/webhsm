...
import {declaraEstarCiente, escolhendoDataConsulta, selecionaHorarioAgenda} from '../../DadosAgendamentoActions'
...
class Doctor extends Component {
                ...
                <Card>
                  ...
                  <CardBody>
                    ...
                    <div style={{
                      ...
                      <Input checked={this.props.checkboxDeclaraCiente} className="form-check-input" type="checkbox" onChange={() => this.props.declaraEstarCiente(this.props.checkboxDeclaraCiente)}/>
                      <Label check="check">
              ...
              <Col xs="12" sm="6" md="6">
                <Card>
                  ...
                  <CardBody style={{
                    ...
                    <Row className="justify-content-center">
                      <Calendar onChange={(value) => this.props.escolhendoDataConsulta(value, this.props.convenio, this.props.medico)} value={this.props.dataConsulta}/>
                    </Row>
                  </CardBody>
                </Card>
              </Col>
  ...
  montaHorariosAgenda() {
    return (this.props.agendasmedico.filter((item, i, ar) => (ar.slice(i + 1, ar.length).filter((item2) => (item2.dtreferencia === item.dtreferencia)).length === 0)).map((agenda, index) => {
      if (index < 3) {
        let dia = moment(agenda.dtreferencia, "DD/MM/YYYY HH:mm").locale('pt-BR');
        return (<div key={index}>
          <Row key={index + "titulo"} className="justify-content-center" style={{
              paddingBottom: 10
            }}>
            <h5>{dia.format("dddd, DD/MM/YYYY")}</h5>
          </Row>
          <Row key={index + "horario"} className="justify-content-center" style={{
              paddingBottom: 20
            }}>
            {
              this.props.agendasmedico.filter((item) => (item.dtreferencia === agenda.dtreferencia)).map((horario, i) => (<Col key={i} xs="12" sm="4" md="2">
                ...
                <Badge style={{
                  ...
                  <Label className="form-check-label" check="check" htmlFor="inline-radio2">
                    {moment(horario.hrinicon, "DD/MM/YYYY HH:mm").format("HH:mm") + " - " + moment(horario.hrtercon, "DD/MM/YYYY HH:mm").format("HH:mm")}
                  </Label>
                </Badge>
              </Col>))
            }
...
const mapStateToProps = state => ({
  ...
  dataConsultafim: state.dadosAgendamento.dataConsultaFim,
...
const mapDispatchToProps = dispatch => bindActionCreators({
  declaraEstarCiente,
  escolhendoDataConsulta,
  selecionaHorarioAgenda
}, dispatch)
